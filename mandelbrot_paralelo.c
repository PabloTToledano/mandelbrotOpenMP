...
/* Pablo Tomás Toledano González, Alejandro Ruiz Huertas
...
int main(int argc, char *argv[])	{
	...
	for (imag = 0; imag <n_imag; imag ++)	{
        ...
        printf("Altura: %d\n",height);
        printf("Anchura: %d\n",width);
        #pragma omp parallel for private(i,j,x,y) //schedule(static, 1000)
            for(i = 0; i < height; i++){
                for(j = 0; j < width; j++) {
                    x = xmin + j/k; 
                    y = ymax - i/k; 
                    imagen[i][j]= mandel_val(x, y, MAX_ITER);
                }
            }
		...
		printf("\n\n Tiempo empleado paralelo %0.2f segundos\n\n",omp_get_wtime()-t);
...
Tiempo rectángulo secuencial: 13.69 s

##### CONCLUSIONES #####
Tomando diferentes valores para los diferentes modos de schedule, podemos sacar varios valores bastante interesantes.
Comparando los tiempos con un tt de 200 en estático y dinámico, podemos observar que hay una diferencia de 0.65 s. 
Esto es previsible, ya que en el schedule dinámico se van reasignando las hebras según van quedando disponibles.
Mientras tanto, en el valor de tt = 500 ésta diferencia va disminuyendo, debido a que el número de reasignaciones va disminuyendo.
También podemos ver que a medida que se aumenta tt, también aumenta el tiempo, ya que el bucle va haciendo más trabajo y se paraleliza menos.
Con tt = 1000, el tiempo se acerca mucho al valor del algoritmo de Mandelbrot secuencial. Esto es debido a que asignamos una carga de trabajo
enorme a la primera hebra, casi igual que la del programa secuencial, por lo que el valor va a ser prácticamente el mismo.
*/